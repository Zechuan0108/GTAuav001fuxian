...
class DesModel(nn.Module):
    ...
    def forward(self, img1=None, img2=None, img3=None):

        if self.share_weights:
            if img1 is not None and img2 is not None:
                # print("Input dtype:", img1.dtype)
                image_features1 = self.model(img1)
                # print("Input dtype:", img2.dtype)
                image_features2 = self.model(img2)
                # print("Input dtype:", img3.dtype)
                image_features3 = self.model(img3)

                return image_features1, image_features2, image_features3
            ...
            if img1 is not None and img2 is not None:
                ...
                image_features3 = self.model2(img3)
                return image_features1, image_features2, image_features3
            elif img1 is not None:
    ...
    def offset_pred(self, img_feature1, img_feature2, img_feature3):
        offset1 = self.MLP(torch.cat((img_feature1, img_feature2), dim=1))
        offset2 = self.MLP(torch.cat((img_feature3, img_feature2), dim=1))
...
if __name__ == '__main__':
    ...
    # model = DesModel(model_name='timm/vit_base_patch14_dinov2.lvd142m', img_size=384)
