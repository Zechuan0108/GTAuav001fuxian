...
def train_with_weight(train_config, model, dataloader, loss_function, optimizer, scheduler=None, scaler=None,
                      recon_weight=0.1, loss_recon=None, train_with_recon=False, with_weight=False):
    ...
    for query, reference, weight, query_bev in bar:
        ...
        if scaler:
            with autocast():
                ...
                if train_with_recon:
                    features1, features1_recon, features2, features2_recon = model(img1=query, img2=reference,
                    ...
                    features1, features2, features3 = model(img1=query, img2=reference, img3=query_bev)

                if torch.cuda.device_count() > 1 and len(train_config.gpu_ids) > 1:
                    if with_weight:
                        loss_sim = loss_function(features1, features2, features3, model.module.logit_scale.exp(),
                                                 weight)
                    else:
                        loss_sim = loss_function(features1, features2, features3, model.module.logit_scale.exp())
                else:
                    if with_weight:
                        loss_sim = loss_function(features1, features2, features3, model.logit_scale.exp(), weight)
                    else:
                        loss_sim = loss_function(features1, features2, features3, model.logit_scale.exp())
...
def train(train_config, model, dataloader, loss_function, optimizer, scheduler=None, scaler=None, num_chunks=1):
    ...
    for query, reference, ids in bar:
        ...
        if train_config.verbose:
            ...
            print(
                f"Iteration {step}/{total_step} took {iter_time:.4f} s, ETA: {eta:.2f} s, loss: {loss}, loss_avg: {loss_avg}, lr: {lr}",
                flush=True)
