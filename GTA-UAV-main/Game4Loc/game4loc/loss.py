...
class WeightedInfoNCE(nn.Module):
    ...
    def forward(self, image_features1, image_features2, image_features3, logit_scale, positive_weights=None):
        # Normalize the image features
        image_features1 = F.normalize(image_features1, dim=-1)
        image_features2 = F.normalize(image_features2, dim=-1)
        image_features3 = F.normalize(image_features3, dim=-1)
        ...
        #########################################################################
        logits_per_image3 = logit_scale * image_features3 @ image_features2.T
        loss1 = self.loss(logits_per_image3, eps)
        loss2 = self.loss(logits_per_image2, eps)
        lossb = (loss1 + loss2) / 2
...
class GroupInfoNCE(nn.Module):
    def __init__(self, group_len, label_smoothing, loss_type, k=3,
    ...
    def forward(self, image_features1, image_features2, logit_scale, positive_weights=None):
        ...
        for loss_type in self.loss_type:
            if loss_type == 'part_slice':
                loss[loss_type] = (self.loss_part_slice(logits_per_image1, G, N) + self.loss_part_slice(
                    logits_per_image2, G, N)) / 2
            elif loss_type == 'whole_slice':
                loss[loss_type] = (self.loss_whole_slice(logits_per_image1, G, N) + self.loss_whole_slice(
                    logits_per_image2, G, N)) / 2
            elif loss_type == 'part_block':
                loss[loss_type] = (self.loss_part_block(logits_per_image1, G, N) + self.loss_part_block(
                    logits_per_image2, G, N)) / 2
            elif loss_type == 'whole_block':
                loss[loss_type] = (self.loss_whole_block(logits_per_image1, G, N) + self.loss_whole_block(
                    logits_per_image2, G, N)) / 2
            elif loss_type == 'contrastive_slice':
                loss[loss_type] = (self.loss_contrastive_slice(logits_per_image1, G, N) + self.loss_contrastive_slice(
                    logits_per_image2, G, N)) / 2
...
class MMWeightedInfoNCE(nn.Module):
    ...
    def forward(self,
        ...
        if self.dimg2simg:
            loss_drone_img_satellite_img = (self.loss(logits_drone_img2satellite_img, eps_weight) + self.loss(
                logits_satellite_img2drone_img, eps_weight)) / 2
        else:
            loss_drone_img_satellite_img = 0.

        if self.dimg2dpc:
            loss_drone_img_drone_pc = (self.loss(logits_drone_img2drone_pc, eps) + self.loss(logits_drone_pc2drone_img,
                                                                                             eps)) / 2
        else:
            loss_drone_img_drone_pc = 0.

        if self.dpc2simg:
            loss_drone_pc_satellite_img = (self.loss(logits_drone_pc2satellite_img, eps_weight) + self.loss(
                logits_satellite_img2drone_pc, eps_weight)) / 2
        else:
            loss_drone_pc_satellite_img = 0.

        if self.ddepth2dimg:
            loss_drone_depth_drone_img = (self.loss(logits_drone_img2drone_depth, eps) + self.loss(
                logits_drone_depth2drone_img, eps)) / 2
        else:
            loss_drone_depth_drone_img = 0.

        if self.ddepth2simg:
            loss_drone_depth_satellite_img = (self.loss(logits_satellite_img2drone_depth, eps_weight) + self.loss(
                logits_drone_depth2satellite_img, eps_weight)) / 2
        else:
            loss_drone_depth_satellite_img = 0.

        if self.ddesc2simg:
            loss_drone_desc_satellite_img = (self.loss(logits_satellite_img2drone_desc, eps_weight) + self.loss(
                logits_drone_desc2satellite_img, eps_weight)) / 2
        else:
            loss_drone_desc_satellite_img = 0.

        if self.ddesc2dimg:
            loss_drone_desc_drone_img = (self.loss(logits_drone_desc2drone_img, eps) + self.loss(
                logits_drone_img2drone_desc, eps)) / 2
        else:
            loss_drone_desc_drone_img = 0.

        if self.dpc2dimg:
            loss_drone_pc_drone_img = (self.loss(logits_drone_pc2drone_img, eps) + self.loss(logits_drone_img2drone_pc,
                                                                                             eps)) / 2
        else:
            loss_drone_pc_drone_img = 0.

        if self.sdesc2simg:
            loss_satellite_desc_satellite_img = (self.loss(logits_satellite_desc2satellite_img, eps) + self.loss(
                logits_satellite_img2satellite_desc, eps)) / 2
        else:
