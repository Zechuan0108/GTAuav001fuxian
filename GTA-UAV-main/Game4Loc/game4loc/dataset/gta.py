...
import os
# 添加项目根目录到 Python 路径
sys.path.insert(0, '/data3/jcx_code/test9.18/GTA-UAV-main')
import numpy as np
...
import json
import sys
from Game4Loc.bev_transform.utils import get_BEV_tensor
from Game4Loc.bev_transform.zhuanhua import correct_tilted_image
...
class GTADatasetTrain(Dataset):
    ...
    def __init__(self,
        ...
        self.pairs = []
        self.pairs_sate2drone_dict = {}  # 无人机→[匹配的卫星图像列表]
        self.pairs_drone2sate_dict = {}  # 卫星→[匹配的无人机图像列表]
    ...
    def __getitem__(self, index):
        ...
        #######################################################################################################
        # query_bev = get_BEV_tensor(query_img, 384, 384, Fov=85 * 2, dty=0, dx=0, dy=0, out=None, device='cpu')
        query_bev = correct_tilted_image(query_img)
        #####################################################################################################
        ...
        if self.transforms_gallery is not None:
            ...
            query_bev = self.transforms_gallery(image=query_bev)['image']

        return query_img, gallery_img, positive_weight, query_bev  # , query_loc_xy/NORM_LOC, gallery_loc_xy/NORM_LOC
                    ...
                    for sate_img in subset_sate_i:
                        ...
                        if sate2drone_inter_set == None:
                            ...
                            sate2drone_inter_set = sate2drone_inter_set.intersection(
                                self.pairs_sate2drone_dict[sate_img])
