...
class Configuration:
    ...
    verbose: bool = True
    gpu_ids: tuple = (1, 3)  # GPU ids for training
    ...
    @property
    def device(self):
        return f'cuda:{self.gpu_ids[0]}'  # 明确主设备为cuda:4
...
def train_script(config):
    ...
    # Model to device
    model.to(f'cuda:{config.gpu_ids[0]}')
...
def parse_args():
    ...
    parser.add_argument('--train_mode', type=str, default='pos_semipos',
                        help='Train with positive or positive+semi-positive pairs')
    ...
    parser.add_argument('--loss_type', type=str, nargs='+', default=['part_slice', 'whole_slice'],
                        help='Loss type for group train')
...
if __name__ == '__main__':
    ...
    val_transforms, train_sat_transforms, train_drone_transforms = \
        get_transforms(img_size, mean=mean, std=std, sat_rot=sat_rot)
    train_dataset = GTADatasetTrain(data_root='/data3/czh_code/gta/GTA-UAV-LR',
                                    pairs_meta_file=config.train_pairs_meta_file,
                                   transforms_query=train_drone_transforms,
                                    transforms_gallery=train_sat_transforms,
                                    group_len=config.group_len,
    ...
    query, gallery, weight = train_dataset[1]
    print(query.shape)
    print(gallery.shape)
    print(weight)
    ####################################################
