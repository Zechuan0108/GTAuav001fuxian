...
import sys
sys.path.append('/data3/czh_code/gta')
from Game4Loc.bev_transform.torch_geometry import euler_angles_to_matrix, get_perspective_transform
...
def get_BEV_tensor(img, Ho, Wo, Fov=170, dty=-20, dx=0, dy=0, dataset=False, out=None, device='cpu'):
    ...
    Hp, Wp = img.shape[0], img.shape[1]  # Panorama image dimensions
    if dty != 0 or Wp != 2 * Hp:
        ty = (Wp / 2 - Hp) / 2 + dty  # Non-standard panorama image completion  dty < 0：图像向上移动显示更多天空区域
    ...
    #     out = out.to(device)
    t3 = time.time()
    #frame.permute(2, 0, 1).unsqueeze(0).float()---(1,c,h,w)   out.unsqueeze(0)---(1, Wo, Ho, 2)
